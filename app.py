...
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_benchmark(symbol, period="1y"):
...
def calculate_theme_index(results, theme_stocks):
...
def render_dashboard_view(results, stock_list):
    ...
    ew_index = calculate_theme_index(results, theme_stocks)
    ...
    for bench_sym in benchmarks:
        bench_prices = fetch_benchmark(bench_sym)
        if bench_prices is not None and len(bench_prices) > 0:
            bench_normalized = (bench_prices / bench_prices.iloc[0]) * 100
    ...
    fig.update_layout(
        ...
        title=f'{selected_theme} Theme vs Benchmarks (Normalized to 100)',
        height=450,
    ...
    index_start = ew_index.iloc[0]
    return_12m = ((index_level / index_start) - 1) * 100

    # Average RSI across theme
    rsi_values = []
    for _, row in theme_stocks.iterrows():
    ...
    if primary_bench in bench_data:
        bench_level = bench_data[primary_bench].iloc[-1]
        bench_start = bench_data[primary_bench].iloc[0]
        bench_return = ((bench_level / bench_start) - 1) * 100
        vs_bench = return_12m - bench_return
    else:
        vs_bench = 0
        bench_return = 0

    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("EW Index Level", f"{index_level:.1f}")
    col2.metric("12M Return", f"{return_12m:+.2f}%")
    col3.metric("RSI(14)", f"{avg_rsi:.1f}")
    col4.metric("Trend Status", trend_status)
    col5.metric(f"vs {primary_bench} (12M)", f"{vs_bench:+.2f}%")

    # Narrative
    st.markdown(f"""
    The equal-weight {selected_theme.lower()} index stands at **{index_level:.1f}**, reflecting a
    **{return_12m:+.1f}%** return over the trailing 12 months. This compares to
    {primary_bench} at **{bench_return:+.1f}%** over the same period.
...
def main():
    ...
    with col_nav:
        view = st.selectbox("", ["Stock Tables", "Dashboard"], label_visibility="collapsed")
        ...
        for i, theme in enumerate(themes):
            with tabs[i]:
                ...
                st.dataframe(
                    ...
                    height=min(600, 40 + len(scorecard) * 35),
                    hide_index=True,
                )
